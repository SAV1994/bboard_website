...
def send_activation_notification(user):
    """Отправляет письма с ссылкой на активацию"""
    if ALLOWED_HOSTS:
...
def get_timestamp_patch(instance, filename):
    """Генератор имён для выгружаемых изображений"""
    return '%s%s' % (datetime.now().timestamp(), splitext(filename)[1])


def send_new_comment_notification(comment):
    """Отправляет письма с уведомлением о новом комментарии"""
    if ALLOWED_HOSTS:
        host = 'http://' + ALLOWED_HOSTS[0]
    else:
        host = 'http://localhost:8000'
